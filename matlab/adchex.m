...
%fs = 16000000/16;                                   %ADC采样频率28000000/16 %16000000/32;200000
fs =800000;                                   %ADC采样频率28000000/16 16000000/32
%bitsize = 16;                                       %AD位数
bitsize = 18;                                       %AD位数
maxdata = 2^bitsize-1;                              %12位ADC的最大代码，跟AD的位数有关
VREF = 20.0; %参考电压2.5V3.298V3.253
VIN = 19.530; %实际输入电压9.5148
%VIN = 24.0;%实际输入电压9.5148
Voffset =0;
%ENOB = ((SINAD-(10*log10(1.5)))+20*log10(VREF/VIN))/(20*log10(2));%增加幅值的修正系数

filename = './test/nb2213_89d84.txt';
%filename = './current/hexout.txt';
...
rsize = floor(size(data, 2)/2);
datax = zeros(1, rsize);
for i = 1:rsize
    temp0 = data(1,2*i-1);
    temp1 = data(1,2*i);
    if(bitsize == 18) ;
    datax(1,i)  = temp0*4 + floor(temp1/(2^14));  %for 18bit
    end
    if(bitsize == 16) ;
    datax(1,i)  = temp0;   %for 16bit
    end
end
...
%w%figure;
%w%plot([-99:100],Voffset-VIN/2+code([(code_len-99):code_len 1:100])*VREF/(2^bitsize-1),'-*');
% code  = code+2048;
...
%code_fft = fft (code(1:code_len).*hft248(code_len)',code_len);    %将采样值做FFT变换,并加blackmanharris窗函数
%code_fft = fft (code(1:code_len).*hft144(code_len)',code_len);    %将采样值做FFT变换,并加blackmanharris窗函数
...
%%plot([1:code_len],code(1:code_len));
%%plot([1:code_len],code);
%%plot([1:code_len],code(1:code_len).*hft248(code_len)','-*');
%plot([1:code_len],code(1:code_len).*hft144(code_len)','-*');
%title('TIME DOMAIN')
%xlabel('SAMPLES');
%ylabel('DIGITAL OUTPUT CODE');
...
semilogx([0:code_len/2-1].*fs/code_len,Dout_dB(1:code_len/2)-maxdB);    %横轴为频率，纵轴为幅值（分贝）
grid on;
...
%tone=rem((har_num*(fin)+1)/code_len,1);
